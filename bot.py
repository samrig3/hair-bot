...
def button_label_for(data: str, callback_query: dict) -> str:
    """Find the human-readable text of the button the user tapped, by matching
    the callback data against the original message's inline keyboard."""
        ...
        for update in updates:
            ...
            try:
                if "message" in update:
                    ...
                    data = cb.get("data")
                    if data:
                        # Show the user what they tapped by editing the original
                        # message (inline button presses otherwise leave no trace).
                        msg = cb.get("message") or {}
                        chat = (msg.get("chat") or {}).get("id")
                        mid = msg.get("message_id")
                        label = button_label_for(data, cb)
                        if chat and mid and label:
                            original = msg.get("text", "")
                            tg.edit_message_text(chat, mid, f"{original}\n\n✓ <b>{label}</b>")
                        handle_callback(data, state)
            except Exception as e:
